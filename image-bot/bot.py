...
class ImageBotCore:
    ...
    async def process_image_job(self, row_num, input_image_path, prompt, ratio, qty_per_loop, total_qty, download_reso="2K"):
        ...
        for i in range(loops):
            ...
            try:
                # -------- CẮM LẠI DÂY ẢNH GỐC (THỰC HIỆN TRƯỚC ĐỂ KHÔNG BỊ PROMPT CHE BÓNG) --------
                ...
                try:
                    ...
                    # Rê chuột và ép click vào menu ba chấm ẩn
                    await p_wrap.locator("button").last.hover()
                    await asyncio.sleep(1) # Giãn nhịp để CPU tản nhiệt
                    await p_wrap.locator("button").last.click(force=True)
                    
                    # Kiên nhẫn chờ React Menu hiện lên (tránh CPU Lag làm mất thao tác)
                    ...
                    print(f"⚠️ Khựng tay lúc gài Ảnh: {e}")
                
                await asyncio.sleep(2) # Cho CPU thời gian nạp cái Chip ảnh vào Prompt
